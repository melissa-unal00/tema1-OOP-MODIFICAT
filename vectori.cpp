...
double operator[](int i) //metoda operator [] transformata in functie friend
{
return v[i];
}

int& vectori:: length (vectori nr_elem) //functia friend length transformata in functie membra
{
    return nr_elem.n;
}


istream& operator >>(istream& in,management& A)  // supraincarcarea lui >>
{
    in>>A.n;
    for (int i=0; i<A.n; i++)
        in>>A.vec[i];
...
ostream& operator<<(ostream& out, management& A) // suspraincarcarea lui <<
{
    out<<A.n;
    for (int i=0; i<A.n; i++)
        out<<A.vec[i];
...
management management::operator=(management C) //supraincarcarea operatorului =
{
n=C.n;
vec= new double[n];
for (int i=0;i<n;i++)
